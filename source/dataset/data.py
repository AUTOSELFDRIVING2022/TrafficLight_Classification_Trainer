...
def get_train_loadersRGB(path, device, batch_size, workers, class_count):
    def to_device(x, y):
        return x.to(device), y.to(device, dtype=torch.int64)
...
def get_train_loadersLight(path, device, batch_size, workers, class_count):
    def to_device(x, y):
        return x.to(device), y.to(device, dtype=torch.int64)
    ...
    weights = 1 / np.array([class_sample_count[y]
                            for y in train_dataset.labels])
    samp = sampler.WeightedRandomSampler(weights, 7 * 200000)
    train_loader = WrappedDataLoader(DataLoader(
        train_dataset, batch_size=batch_size, sampler=samp, num_workers=workers), to_device)
    #train_loader = WrappedDataLoader(DataLoader(
    #    train_dataset, batch_size=batch_size, sampler=None, num_workers=workers), to_device)
    valid_loader = WrappedDataLoader(DataLoader(
        valid_dataset, batch_size=batch_size, shuffle=False, num_workers=workers), to_device)
...
def get_test_loaderRGB(path, device, gray=True):
    def preprocess(x, y):
        return x.to(device), y.to(device, dtype=torch.int64)
...
def get_test_loadersLight(path, device, gray=True):
    def preprocess(x, y):
        return x.to(device), y.to(device, dtype=torch.int64)
    ...
    test_dataset = PickledDataset(
        path+'/test.p', transform=get_test_transforms())
    test_loader = WrappedDataLoader(DataLoader(
        test_dataset, batch_size=1, shuffle=False),preprocess)
